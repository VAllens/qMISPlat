...
(function ($) {
	...
	myflow.rect = function(o, r) {
        ...
        // 事件处理--------------------------------
        $([_rect.node, _text.node, _name.node, _img.node]).bind('dblclick', function () {
            PhaseOndblclick(_this, _id, _bbox, _text, _rect, _name, _img, o);
        });
			...
			switch (_o.showType) {
			    ...
			    case 'imageStart':
			        if (_o.IsCurActive) {
			            //流程实例当前节点
			            _text.attr({
			                x: rx + _o.img.width + (rw - _o.img.width) / 2 +30,
			                y: ry + (rh - myflow.config.lineHeight) / 2
                            + myflow.config.lineHeight - 10,
			                fill: "red"
			            }).show();// 文本
			        }
			        else {
    ...
    myflow.path = function (o, r, from, to) {
        ...
        var pathType = 1;//1:提交 2回退
        try {
            if (o.PathType == 1)
            {
                pathType = 1;
            }
            else if (o.PathType == 2) {
                pathType = 2;
            }
        }
        catch (e) { }     
        try {
            if (o.props.PathType.value == "1") {
                pathType = 1;
            }
            else if (o.props.PathType.value == "2") {
                pathType = 2;
            }
        }
        catch (e) { }   
        ...
        if (_o.IsOverOrActive) {
            ...
            if (pathType == 1) {
                ...
                //回退
                if (myflow.config.editable == false) {
                    //监控状态下
                    _path = _r.path(_o.attr.path.path).attr(_o.attr.path);
                    _arrow = _r.path(_o.attr.arrow.path).attr(_o.attr.arrow);
                }
                else {
                    _path = _r.path(_o.attr.path.path).attr(_o.attr.pathRed);
                    _arrow = _r.path(_o.attr.arrow.path).attr(_o.attr.arrowRed);
                }
            }
        }
        ...
        // 事件处理--------------------
        $([_path.node, _arrow.node, _text.node]).bind("dblclick", function () { LinkOndblclick(_from, _to,_path); });
	...
	myflow.props = function(o, r) {
		...
		var showpropsHandler = function(e, props, src) {
			...
			_pdiv.show();
			for (var k in props) {
				_tb.append('<tr style="height:50px;"><th>' + props[k].label + '</th><td><div id="p'
	...
	myflow.editors = {
		textEditor : function() {
			...
			this.init = function(props, k, div, src, r) {
                ...
                $('<input  style="width:100%;" class="layui-input"/>').val(_src.text()).change(
						function() {
    ...
    myflow.init = function (c, o) {
		...
		var removeHandler = function(e, src) {
			...
			if (src.getId().substring(0, 4) == 'rect') {
                ...
                myFlow_AllStates[src.getId()] = null;
                src.remove();
		...
		$(_r).bind('addrect', function(e, type, o) {
            ...
            myFlow_AllStates[rect.getId()] = rect;
            CPFlowGlobal_IsNeedSave = true;
		});
		// 添加路径_提交
        var addpathHandler = function (e, from, to) {
            ...
            $.each(_paths, function (nIndex, nObj) {
                ...
                //console.log(nObj.from().getId()); 
                if (
                    (nObj.from().getId() == from.getId() && nObj.to().getId() == to.getId() && nObj.getprops().PathType.value =="1")
                    ||
                    (nObj.from().getId() == to.getId() && nObj.to().getId() == from.getId() && nObj.getprops().PathType.value == "1")
                )
                {
                    bExists = true;
                    return;
            ...
            if (bExists)
                return;
			var path = new myflow.path({PathType:1}, _r, from, to);
            _paths[path.getId()] = path;
            CPFlowGlobal_IsNeedSave = true;
            
        };
        // 添加路径_驳回
        var addpathfallbackHandler = function (e, from, to) {
            //看下这两个节点间有没有联系，如果有，则不允许再画了
            var bExists = false;
            $.each(_paths, function (nIndex, nObj) {
                //console.log(nObj.from().getId()); 
                if (
                    (nObj.from().getId() == from.getId() && nObj.to().getId() == to.getId() && nObj.getprops().PathType.value == "2")
                    ||
                    (nObj.from().getId() == to.getId() && nObj.to().getId() == from.getId() && nObj.getprops().PathType.value == "2")
                ) {
                    bExists = true;
                    return;
                }
            ...
            if (bExists)
                return;
            var path = new myflow.path({ PathType: 2}, _r, from, to);
            _paths[path.getId()] = path;
            CPFlowGlobal_IsNeedSave = true;

        };
        $(_r).bind('addpath', addpathHandler);
        $(_r).bind('addpathfallback', addpathfallbackHandler);
